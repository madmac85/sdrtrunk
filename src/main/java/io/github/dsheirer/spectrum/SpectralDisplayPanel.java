...
/**
 * Spectral Display Panel provides a hardware-accelerated frequency component 
 * display (via JavaFX) with a historical waterfall and a transparent overlay.
 * Optimized for Windows 11 using Direct3D and FlatLaf.
 */
        ...
        implements Listener<INativeBuffer>, ISourceEventProcessor, IDFTWidthChangeProcessor
{
    ...
    // Phase 5: Transition to JavaFX accelerated components
    private JFXPanel mSpectrumFxPanel;
    private JFXPanel mWaterfallFxPanel;
    ...
    public SpectralDisplayPanel(PlaylistManager playlistManager, SettingsManager settingsManager, DiscoveredTunerModel discoveredTunerModel)
    {
        ...
        // Note: These must be updated to handle JavaFX rendering internally or be replaced by FX Scenes
    ...
    private void loadSettings()
    {
        ...
        if(rawSize != null)
        {
            try { size = DFTSize.valueOf(rawSize); } catch(Exception e) { /* Default */ }
        }

        if(size == null) { size = DFTSize.FFT04096; }
    ...
    private void init()
    {
        ...
        // Phase 5: Set up JavaFX Panels for Hardware Acceleration
        mSpectrumFxPanel = new JFXPanel();
        mWaterfallFxPanel = new JFXPanel();

        // Initialize JavaFX Scenes
        Platform.runLater(() -> {
            // Placeholder for accelerated Spectrum Canvas
            StackPane spectrumRoot = new StackPane();
            mSpectrumFxPanel.setScene(new Scene(spectrumRoot));

            // Placeholder for accelerated Waterfall Canvas
            StackPane waterfallRoot = new StackPane();
            mWaterfallFxPanel.setScene(new Scene(waterfallRoot));
        });
        ...
        // Layers: Spectrum (Background) and Overlay (Transparent Interactive Layer)
        mLayeredPanel.add(mSpectrumPanel, JLayeredPane.DEFAULT_LAYER);
        mLayeredPanel.add(mOverlayPanel, JLayeredPane.PALETTE_LAYER);
        ...
        double totalHeight = mLayeredPanel.getPreferredSize().getHeight() + mWaterfallPanel.getPreferredSize().getHeight();

        mLayeredPanel.setPreferredSize(new Dimension((int)mLayeredPanel.getPreferredSize().getWidth(), (int)(totalHeight / 2.0d)));
        mWaterfallPanel.setPreferredSize(new Dimension((int)mWaterfallPanel.getPreferredSize().getWidth(), (int)(totalHeight / 2.0d)));
    ...
    public class MouseEventProcessor extends MouseInputAdapter
    {
        ...
        @Override public void mouseMoved(MouseEvent event) { update(event); }
        ...
        {
            if(e.getComponent() == mOverlayPanel) { mOverlayPanel.setCursorVisible(true); }
            else { mWaterfallPanel.setCursorVisible(true); }
        }
        ...
        @Override public void mouseClicked(MouseEvent event)
        {
            if(SwingUtilities.isRightMouseButton(event))
            {
                ...
                if(event.getComponent() == mOverlayPanel)
                {
                    ...
                    if(!channels.isEmpty()) { contextMenu.add(new JSeparator()); }
                }
                ...
                for(DFTSize width : DFTSize.values()) { fftWidthMenu.add(new DFTSizeItem(SpectralDisplayPanel.this, width)); }
                ...
                frameRateMenu.add(new FrameRateItem(mComplexDftProcessor, 60)); // Modern Windows 11 high refresh

                JMenu fftWindowType = new JMenu("Window Type");
                displayMenu.add(fftWindowType);
                for(WindowType type : WindowType.values()) { fftWindowType.add(new FFTWindowTypeItem(mComplexDftProcessor, type)); }
                ...
                for(DiscoveredTuner discoveredTuner : mDiscoveredTunerModel.getAvailableTuners())
                {
                    ...
                    {
                        if(!separatorAdded) { contextMenu.add(new JSeparator()); separatorAdded = true; }
                        contextMenu.add(new ShowTunerMenuItem(mDiscoveredTunerModel, discoveredTuner.getTuner()));
                    }
                }

                if(event.getComponent() == mOverlayPanel) { contextMenu.show(mOverlayPanel, event.getX(), event.getY()); }
                else { contextMenu.show(mWaterfallPanel, event.getX(), event.getY()); }
            }
        }
    }
    ...
    public class ZoomItem extends JSlider
    {
        ...
        public ZoomItem(long frequency, double windowOffset)
        {
            ...
            for(int i=0; i<=6; i++) { labels.put(i, new JLabel((int)Math.pow(2, i) + "x")); }
            ...
            addChangeListener(e -> setZoom(getValue(), mFrequency, mWindowOffset));
        }
    }
