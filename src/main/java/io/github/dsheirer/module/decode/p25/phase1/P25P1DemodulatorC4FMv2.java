 ...
 */
public class P25P1DemodulatorC4FMv2
{
    ...
    // === C4FM V2 Improvements ===

    // Gardner TED state
    private float mPreviousSoftSymbol = 0.0f;
    private double mPreviousSymbolBufferOffset = 0.0;
     ...
     */
    public P25P1DemodulatorC4FMv2(P25P1MessageFramer messageFramer, FeedbackDecoder feedbackDecoder)
    {
    ...
    public void setGardnerBandwidth(float bandwidth)
    {
        ...
        double theta = bandwidth / (damping + 1.0 / (4.0 * damping));
        double d = 1.0 + 2.0 * damping * theta + theta * theta;
        mGardnerAlpha = (4.0 * damping * theta) / d;
        mGardnerBeta = (4.0 * theta * theta) / d;
    }

    /**
     * Sets the AFC tracking alpha (slow 1st-order filter).
     * @param alpha tracking speed (0.001-0.1, typical 0.01)
     */
     ...
     */
    public void setAdaptiveThresholdsEnabled(boolean enabled)
    {
        mAdaptiveThresholdsEnabled = enabled;
    }
        ...
        while(samplesPointer < samples.length)
        {
            ...
            while(bufferPointer < bufferReloadThreshold)
            {
                ...
                if(samplePoint < 1)
                {
                    ...
                    // V2: Apply AFC correction (remove frequency offset bias)
                    if(mAfcEnabled)
                    {
                    ...
                    if(mGardnerEnabled && fineSync && mPreviousSymbolBufferOffset > 0)
                    {
                        // Interpolate mid-symbol sample (halfway between previous and current)
                        double midOffset = (mPreviousSymbolBufferOffset + bufferPointer + samplePoint) / 2.0;
                        int midIntegral = (int)Math.floor(midOffset);
                        double midMu = midOffset - midIntegral;
                        if(midIntegral >= 0 && midIntegral < mBuffer.length - 1)
                        {
                            float midSample = mEqualizer.getEqualizedSymbol(
                                mBuffer[midIntegral], mBuffer[midIntegral + 1], midMu);
                            if(mAfcEnabled)
                            {
                                midSample -= mAfcOffset;
                            }

                            // Gardner error: e(n) = [y(n-1) - y(n)] * y(n-1/2)
                            float gardnerError = (mPreviousSoftSymbol - softSymbol) * midSample;

                            // 2nd-order loop filter
                            mGardnerW += mGardnerBeta * gardnerError;
                            double adjustment = mGardnerAlpha * gardnerError + mGardnerW;

                            // Clamp adjustment
                            adjustment = Math.max(-GARDNER_MAX_ADJUSTMENT, Math.min(GARDNER_MAX_ADJUSTMENT, adjustment));
                    ...
                    // Store for next Gardner iteration
                    mPreviousSoftSymbol = softSymbol;
                    mPreviousSymbolBufferOffset = bufferPointer + samplePoint;
                    ...
                    if(correctionCandidate.hasValidTiming())
                    {
                                ...
                                (correctionCandidate.hasHighQualityOptimizationScore() && mMessageFramer.isComplete()))
                        {
                            ...
                            // V2: Update adaptive threshold amplitude estimate from sync score
                            if(mAdaptiveThresholdsEnabled)
                            {
                                float bestScore = Math.max(correctionCandidate.getDetectionScore(),
                                    correctionCandidate.getOptimizationScore());
                                updateAmplitudeEstimate(bestScore);
                            }
    ...
    public void setSamplesPerSymbol(float samplesPerSymbol)
    {
        ...
        // V2: Initialize Gardner TED loop filter with default bandwidth
        if(mGardnerAlpha == 0.0)
        {
    ...
    /**
     * Decodes a sample to a symbol using adaptive thresholds scaled by estimated signal amplitude.
     * @param sample in radians
     * @return symbol decision
     */
    public Dibit toSymbolAdaptive(float sample)
    {
        float threshold = SOFT_SYMBOL_QUADRANT_BOUNDARY * mAmplitudeEstimate;
        // Clamp to prevent pathological behavior
        threshold = Math.max((float)(Math.PI / 4.0), Math.min((float)(Math.PI * 3.0 / 4.0), threshold));

        if(sample > 0)
        {
            return sample > threshold ? Dibit.D01_PLUS_3 : Dibit.D00_PLUS_1;
        }
        else
        {
            return sample < -threshold ? Dibit.D11_MINUS_3 : Dibit.D10_MINUS_1;
        }
    }

    /**
     * Updates the amplitude estimate from a sync correlation score.
     * @param syncScore from the correlation detector
     */
    private void updateAmplitudeEstimate(float syncScore)
    {
        // Score = sum of 24 symbols * ideal_phase. Ideal score = 24 * (3pi/4)^2 ~= 133
        // Amplitude = actual_score / ideal_score
        float idealScore = 24.0f * Dibit.D01_PLUS_3.getIdealPhase() * Dibit.D01_PLUS_3.getIdealPhase();
        float amplitude = syncScore / idealScore;
        amplitude = Math.max(AMPLITUDE_MIN, Math.min(AMPLITUDE_MAX, amplitude));
        // Smooth update
        mAmplitudeEstimate = 0.7f * mAmplitudeEstimate + 0.3f * amplitude;
    }
